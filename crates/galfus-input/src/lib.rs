...
use galfus_types::{ConnectorId, RealmId, SurfaceId};
use glam::Vec2;
...
pub struct ResolvedInputCapture {
    ...
    pub target_id: Option<galfus_realm_core::TargetId>,
}
...
pub struct InputRoutingCache {
    ...
    pub connector_targets: HashMap<ConnectorId, galfus_realm_core::TargetId>,
    pub layer_camera_by_key: HashMap<(u32, galfus_realm_core::TargetId), Option<u32>>,
...
pub struct InputRoutingState {
    ...
    pub focus_targets: HashMap<u32, galfus_realm_core::TargetId>,
...
pub fn resolve_captured_connector(
    ...
    captures
        ...
        .map(|capture| ResolvedInputCapture {
            ...
            target_id: capture.target_id.map(galfus_realm_core::TargetId),
        })
}

pub fn resolve_focus_target(
    focus_targets: &HashMap<u32, galfus_realm_core::TargetId>,
    window_id: u32,
) -> Option<galfus_realm_core::TargetId> {
    focus_targets.get(&window_id).copied()
}

pub fn resolve_connector_for_target(
    connectors: Option<&Vec<InputRoutingConnectorHit>>,
    target_id: galfus_realm_core::TargetId,
) -> Option<ConnectorId> {
    let connectors = connectors?;
    for connector in connectors {
