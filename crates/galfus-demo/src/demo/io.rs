...
use galfus_core::core::cmd::{CommandResponseEnvelope, EngineCmd, EngineCmdEnvelope, EngineEvent};
...
pub fn send_commands(cmds: Vec<EngineCmd>) -> GalfusResult {
    ...
    core::galfus_send_queue(data.as_ptr(), data.len())
}

pub fn receive_responses() -> Vec<CommandResponseEnvelope> {
    let mut ptr = std::ptr::null();
    let mut len: usize = 0;
    let result = core::galfus_receive_queue(&mut ptr, &mut len);

    if result != GalfusResult::Success || len == 0 {
        return Vec::new();
    }
...
pub fn receive_events() -> Vec<EngineEvent> {
    let mut ptr = std::ptr::null();
    let mut len: usize = 0;
    let result = core::galfus_receive_events(&mut ptr, &mut len);
