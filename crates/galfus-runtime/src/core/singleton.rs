...
pub fn validate_thread() -> Result<(), GalfusResult> {
    let current_id = thread::current().id();
    let main_id = MAIN_THREAD_ID.get().ok_or(GalfusResult::NotInitialized)?;

    if &current_id != main_id {
        return Err(GalfusResult::WrongThread);
    }
...
pub fn with_engine<F, R>(f: F) -> Result<R, GalfusResult>
where
    F: FnOnce(&mut EngineState) -> R,
{
    ...
    ENGINE_INSTANCE.with(|cell| {
        ...
        let engine_state = opt.as_mut().ok_or(GalfusResult::NotInitialized)?;
        Ok(f(&mut engine_state.state))
    })
}

/// Execute a closure with mutable access to the entire engine singleton
pub fn with_engine_singleton<F, R>(f: F) -> Result<R, GalfusResult>
where
