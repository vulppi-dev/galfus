...
pub fn engine_cmd_window_create_async(
    ...
    let bootstrap_target =
        galfus_platform::plan_web_render_bootstrap_target(args.window_id, window_size);

    let win_id = args.window_id;
    let canvas_clone = canvas.clone();
    spawn_local(async move {
        let bootstrap_plan = galfus_runtime::plan_render_bootstrap(false, bootstrap_target);
        let instance = galfus_render::create_default_instance();
        let surface =
