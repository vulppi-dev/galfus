...
pub(crate) fn map_realms_to_windows(universal: &UniversalState) -> HashMap<RealmId, u32> {
    ...
    let realm_output_surfaces: HashMap<_, _> = universal
        ...
        .collect();
    galfus_render::map_realms_to_windows(
        &existing_realms,
        &layer_windows,
        &presents,
...
pub(crate) fn collect_present_sizes(
    ...
    let window_sizes = windows
        ...
        .collect();
    let _ = galfus_render::update_present_size_cache(&presents, &window_sizes, cache, cache_hash);
}
...
pub(crate) fn collect_surface_views(
    ...
    let surface_requests: Vec<_> = universal
        ...
        .entries
        .iter()
        .map(|(surface_id, entry)| galfus_render::SurfaceTargetRequest {
            surface_id: *surface_id,
            declared_size: entry.value.size,
            is_onscreen: entry.value.kind == crate::core::realm::SurfaceKind::Onscreen,
        })
        .collect();
    let resolved_targets = galfus_render::plan_surface_targets(&surface_requests, present_sizes);
...
pub(crate) fn resolve_realm_surface(
    ...
    let realm_output_surfaces: HashMap<_, _> = universal
        ...
        .map(|(realm_id, entry)| (*realm_id, entry.value.output_surface))
        .collect();
    galfus_render::resolve_realm_surface(&realm_output_surfaces, realm_id)
}
...
pub(crate) fn ensure_surface_target<'a>(
    ...
    galfus_render::ensure_surface_target(device, surface_targets, surface_id, size, format)
}
