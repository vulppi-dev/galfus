...
pub(super) fn apply_realm_environment_bindings(
    ...
    let layers: Vec<_> = universal
        ...
        .flat_map(|layer| {
            if layer.enabled_camera_ids.is_empty() {
                vec![galfus_render::EnvironmentLayerBinding {
                ...
                layer
                    .enabled_camera_ids
                    .iter()
                    .map(|camera_id| galfus_render::EnvironmentLayerBinding {
                        target_id: layer.target_id,
    ...
    let plan = galfus_render::plan_realm_environment_bindings(&layers);
...
pub(super) fn should_render_realm(
    ...
    galfus_render::should_render_realm(&mut entry.value, frame_index)
}
...
pub(super) fn apply_target_size_requests(
    ...
    let update_requests: Vec<_> = requests
        ...
        .filter_map(|(target_id, requested_size)| {
            ...
            let target = engine_state
                ...
                .entries
                .get(&target_id)?;
            Some(galfus_render::TargetSizeUpdateRequest {
                target_id,
                kind: target.kind,
    ...
    for update in galfus_render::plan_target_size_updates(&update_requests) {
...
pub(super) fn build_target_surface_map(
    ...
    let target_kinds = targets
        ...
        .collect();
    galfus_render::build_target_surface_map(&target_kinds, auto_links)
}
    ...
    for render_state in render_states.values_mut() {
        let next_sources: Vec<_> = target_texture_binds
            ...
            .filter_map(|(texture_id, binding)| {
                ...
                Some(galfus_render::ExternalTextureSource {
            ...
            .collect();
        let plan = galfus_render::plan_external_texture_refresh(
...
pub(super) fn collect_window_camera_target_sizes(
    ...
    let targets = universal
        ...
        .collect();
    galfus_render::collect_window_camera_target_sizes(
