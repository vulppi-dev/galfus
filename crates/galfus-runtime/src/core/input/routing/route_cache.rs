...
pub(super) fn rebuild_input_routing_cache(universal: &mut UniversalState) {
    ...
    let topology_hash = galfus_input::compute_input_topology_hash(&snapshot);
    if universal.interaction.input_routing.cache.topology_hash == topology_hash {
        return;
    }
    universal.interaction.input_routing.cache = galfus_input::build_input_routing_cache(&snapshot);
}

fn build_input_routing_topology_snapshot(
    universal: &UniversalState,
) -> galfus_input::InputRoutingTopologySnapshot {
    let realms = universal
        .composition
        .realms
        .entries
        .iter()
        .map(|(realm_id, entry)| galfus_input::InputRoutingRealmOutput {
            realm_id: *realm_id,
    ...
    let presents = universal
        ...
        .entries
        .values()
        .map(|entry| galfus_input::InputRoutingPresentBinding {
    ...
    let target_order = universal
        ...
        .iter()
        .enumerate()
        .map(|(index, target_id)| galfus_input::InputRoutingTargetRank {
            target_id: *target_id,
            rank: index as i32,
        })
        .collect();
    ...
    let auto_links = universal
        ...
        .filter_map(|((_, target_id), link)| {
            link.connector_id
                .map(|connector_id| galfus_input::InputRoutingAutoLinkRecord {
                    target_id: *target_id,
                    connector_id,
                })
        })
    ...
    let layer_cameras = universal
        ...
        .iter()
        .map(
            |((realm_id, target_id), layer)| galfus_input::InputRoutingLayerCameraRecord {
                realm_id: *realm_id,
                target_id: *target_id,
    ...
    let connectors = universal
        ...
        .filter_map(|(connector_id, entry)| {
            universal
                ...
                .map(|surface| galfus_input::InputRoutingConnectorRecord {
    ...
    let surfaces = universal
        ...
        .iter()
        .map(
            |(surface_id, entry)| galfus_input::InputRoutingSurfaceSizeRecord {
