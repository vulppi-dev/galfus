...
fn to_render_preset(preset: ShaderMaterialPreset) -> vulfram_render::MaterialShaderBasePreset {
    match preset {
...
fn to_render_shader_type(shader_type: MaterialShaderType) -> vulfram_render::MaterialShaderType {
    match shader_type {
        MaterialShaderType::Model => vulfram_render::MaterialShaderType::Model,
        MaterialShaderType::Particle => vulfram_render::MaterialShaderType::Particle,
    }
}
...
fn compile_material_program(
    ...
    let compiled = vulfram_render::compile_material_shader_spec(&spec)?;
    let frame_index = engine.runtime.frame_index();
    engine
        .universal_state
        .scene
    ...
    let active_hashes: HashSet<u64> = engine
        ...
        .collect();
    if engine.universal_state.scene.material_program_cache.len() > MATERIAL_PROGRAM_CACHE_MAX_ENTRIES_SOFT {
        engine
            .universal_state
            .scene
            .material_program_cache
            .retain(|key, value| {
                ...
                let within_window =
                    frame_index.saturating_sub(last_used) <= MATERIAL_PROGRAM_CACHE_MAX_UNUSED_FRAMES;
        ...
        engine
            ...
            .scene
            .material_program_cache_last_used_frame
            .retain(|key, _| engine.universal_state.scene.material_program_cache.contains_key(key));
    }
    Ok(compiled)
}
...
fn builtin_shader_source(preset: ShaderMaterialPreset) -> &'static str {
    match preset {
        ShaderMaterialPreset::Standard => {
            vulfram_render::builtin_material_source(vulfram_render::MaterialShaderBasePreset::Standard)
        }
        ShaderMaterialPreset::Pbr => {
            vulfram_render::builtin_material_source(vulfram_render::MaterialShaderBasePreset::Pbr)
        }
    }
}
...
fn ensure_fallback_material_instance(engine: &mut EngineState) {
    ...
    pack_standard_material(MATERIAL_FALLBACK_ID, &StandardOptions::default(), &mut fallback);
...
fn definition_by_id(engine: &mut EngineState, definition_id: u32) -> Option<MaterialDefinitionRecord> {
    bootstrap_builtin_material_definitions(engine);
    engine
        .universal_state
        .scene
...
fn sync_material_from_definition(record: &mut ShaderMaterialRecord, definition: &MaterialDefinitionRecord) {
...
pub fn engine_cmd_material_definition_create(
        ...
        .contains_key(&args.definition_id)
    {
        return CmdResultMaterialDefinition {
            success: false,
            message: format!("Material definition with id {} already exists", args.definition_id),
        };
    }
...
pub fn engine_cmd_material_definition_update(
    ...
    let Some(current) = definition_by_id(engine, args.definition_id) else {
        return CmdResultMaterialDefinition {
            success: false,
            message: format!("Material definition with id {} not found", args.definition_id),
        };
    };
...
pub fn engine_cmd_material_definition_dispose(
    ...
    if args.definition_id == MATERIAL_DEFINITION_STANDARD_ID || args.definition_id == MATERIAL_DEFINITION_PBR_ID {
        return CmdResultMaterialDefinition {
            success: false,
...
pub fn engine_cmd_material_update(
    ...
    if let Some(opts) = &args.options {
        match opts {
            MaterialOptions::Standard(opts) => pack_standard_material(args.material_id, opts, record),
