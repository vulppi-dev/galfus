...
pub fn process_ui_input(engine: &mut EngineState) {
    ...
    let mut scratch = std::mem::take(&mut engine.universal_state.interaction.ui.input_scratch);
        ...
        match event {
            ...
            EngineEvent::Window(WindowEvent::OnScaleFactorChange {
                ...
                if let Some(realms) = realms_by_window.get(window_id) {
                    for realm_id in realms {
                        if let Some(realm) =
                            ensure_realm(&mut engine.universal_state.interaction.ui, *realm_id)
                        {
    ...
    for (realm_id, modifiers) in modifier_updates.drain(..) {
        if let Some(realm) = ensure_realm(&mut engine.universal_state.interaction.ui, realm_id) {
    ...
    for realm in engine.universal_state.interaction.ui.realms.values_mut() {
        realm.profile.input_routing_ms = input_ms;
    }

    engine.universal_state.interaction.ui.input_scratch = scratch;
}
...
fn resolve_pointer_realm(
    ...
    let dispatch = resolve_traced_pointer_dispatch(
        &engine.universal_state.interaction.ui.documents,
...
fn realm_output_size(universal: &UniversalState, realm_id: RealmId) -> Option<glam::UVec2> {
    let realm = universal.composition.realms.get(realm_id)?;
...
fn collect_ui_realms_by_window(engine: &EngineState) -> HashMap<u32, Vec<RealmId>> {
    ...
    for ((layer_realm_id, layer_target_id), _layer) in
    ...
    for (window_id, realm_id) in &engine.universal_state.targets.host_realm_index {
        let is_ui_realm = engine
            .universal_state
            .composition
            .realms
