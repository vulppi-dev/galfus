...
pub(crate) fn engine_process_batch(
    ...
    for pack in batch {
        ...
        let was_deferred = engine.deferred_cmd_meta.contains_key(&deferred_key);
        ...
        let Some(last_response) = engine.response_queue.last().cloned() else {
            continue;
        };
            ...
            super::defer::classify_failed_response(engine, &deferred_cmd, &last_response.response)
        {
            if failure_kind == super::defer::DeferredFailureKind::Transient {
                ...
                if super::defer::should_drop_deferred(attempts, age_frames) {
                    let _ = engine.response_queue.pop();
                    ...
                    engine.deferred_cmd_meta.remove(&deferred_key);
                    ...
                    let _ = engine.response_queue.pop();
                    engine.deferred_cmd_queue.push(EngineCmdEnvelope {
                        id: command_id,
                        cmd: deferred_cmd,
                    });
