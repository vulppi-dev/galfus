...
import type { ReadonlyMat4 } from './mat4';
import type { ReadonlyQuat } from './quat';
import * as vectorMath from './common';

export type vec4 = [number, number, number, number] | IndexedCollection;
export type Vec4 = vec4;
export type ReadonlyVec4 = readonly [number, number, number, number] | IndexedCollection;
...
export function round(out: vec4, a: ReadonlyVec4): vec4 {
  out[0] = vectorMath.round(a[0]);
  out[1] = vectorMath.round(a[1]);
  out[2] = vectorMath.round(a[2]);
  out[3] = vectorMath.round(a[3]);
  return out as Vec4;
}
...
export function random(out: vec4, scale: number): vec4 {
  ...
  rand = vectorMath.RANDOM();
  v1 = rand * 2 - 1;
  v2 = (4 * vectorMath.RANDOM() - 2) * Math.sqrt(rand * -rand + rand);
  s1 = v1 * v1 + v2 * v2;

  rand = vectorMath.RANDOM();
  v3 = rand * 2 - 1;
  v4 = (4 * vectorMath.RANDOM() - 2) * Math.sqrt(rand * -rand + rand);
  s2 = v3 * v3 + v4 * v4;
...
export const forEach = (function () {
  ...
  return function <T>(
    ...
    offset = 0,
    count = 0,
    fn: (out: vec4, vector: vec4, arg: T) => vec4,
    arg: T
  ): number[] {
    let i, l;
