...
export function is3DKeyJustPressed(
  worldId: World3DId,
  keyCode: number,
): boolean {
...
export function is3DKeyJustReleased(
  worldId: World3DId,
  keyCode: number,
): boolean {
...
export function get3DWindowPosition(worldId: World3DId): [number, number] {
...
export function get3DPointerTargetPosition(
  ...
  return (
    getRoutedPointerSnapshotByWorld(asWorldNumber(worldId))
...
export function get3DPointerTargetSize(
  ...
  return (
    getRoutedPointerSnapshotByWorld(asWorldNumber(worldId))?.pointerTargetSize ??
    null
  );
}
...
export function get3DPointerTargetId(worldId: World3DId): number | null {
  return (
    getRoutedPointerSnapshotByWorld(asWorldNumber(worldId))?.pointerTargetId ??
    null
  );
}
...
export function get3DPointerTargetUv(
  ...
  return (
    getRoutedPointerSnapshotByWorld(asWorldNumber(worldId))?.pointerTargetUv ??
    null
  );
}
...
export function get3DScrollDelta(worldId: World3DId): [number, number] {
  return getScrollDelta(asWorldNumber(worldId));
}

/** Returns gamepad events mirrored in the current frame. */
export function get3DGamepadEvents(
  worldId: World3DId,
): ReturnType<typeof getGamepadEvents> {
...
export function get3DGamepadAxis(
  worldId: World3DId,
  gamepadId: number,
  axis: number,
): number {
...
export function get3DLastSystemError(
  worldId: World3DId,
): ReturnType<typeof getLastSystemError> {
  return getLastSystemError(asWorldNumber(worldId));
}

/** Returns all system events mirrored in the current frame. */
export function get3DSystemEvents(
  worldId: World3DId,
): ReturnType<typeof getSystemEvents> {
