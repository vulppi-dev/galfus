...
- World mounting is async-friendly internally; for strict boot sequencing you can poll with `waitWorldReady(...)` from `@galfus/engine/mount`.
...
- Internally, engine-side math defaults are initialized through `@galfus/engine/math`, with the vendored math helpers bundled directly in the engine package, so transforms and vector-shaped fallback values stay consistent without repeating array literals by hand.
